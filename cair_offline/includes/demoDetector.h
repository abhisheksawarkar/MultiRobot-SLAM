  ...
  (std::vector<Matrix> &M,std::vector<int> &index1, std::vector<int> &index2, const std::string &name, const FeatureExtractor<TDescriptor> &extractor)
{
  ...
  int count = 0;
  ofstream withoutinlier,withinlier;
  withoutinlier.open("data/loops_without_inliers.g2o");
  withinlier.open("data/loops_with_inliers.g2o");
  ...
  comment = "#loop closure - \n";
  loop_write_done = false;
  int loopwithinlier =0;
  // go
  for(unsigned int i = 0; i < filenames.size(); ++i)
  {
    pthread_mutex_lock(&myMutex1);
    dloop_wait_flag = true;
    ...
    if(result.detection())
    {
      ...
      if(to_consider==true)
      {
          ...
          //mywriter.my_write_file <<  ss + "# Done loop closure\n";
          withinlier << ss;
          M.push_back(myviso.relate);
          loopwithinlier++;
      }
      if(to_consider==true || to_consider==false)
      {
          ss = my_for_g2o_edge(i,result.match,myviso.relate);
          withoutinlier << ss;
          //M.push_back(myviso.relate);
    ...
    pthread_mutex_lock(&myMutex1);
    dloop_wait_flag = false;
    pthread_mutex_unlock(&myMutex1);

    while(viso_wait_flag)
    {
    }
  ...
  withoutinlier << "# Loops without inliers: " << count << endl;
  mywriter.my_write_file << "# Loops with inliers: " << loopwithinlier << endl;
  loop_write_done = true;
